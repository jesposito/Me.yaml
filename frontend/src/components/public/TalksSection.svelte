				...
				<article class="card p-4 animate-fade-in">
					<div class="flex items-start gap-4">
						...
						<div class="flex-1 min-w-0">
							<h3 class="font-semibold text-gray-900 dark:text-white">{talk.title}</h3>
							<div class="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-gray-600 dark:text-gray-400">
								{#if talk.event}
									<span>{talk.event}</span>
								{/if}
								{#if talk.date}
									<span>{formatDate(talk.date, { month: 'short', year: 'numeric' })}</span>
								{/if}
								{#if talk.location}
			...
			{#each items as talk (talk.id)}
				<article class="card overflow-hidden animate-fade-in">
					{#if talk.video_url}
						...
						{#if embed}
							<div class="aspect-video bg-gray-900">
								{#if embed.type === 'youtube'}
									<iframe src="https://www.youtube.com/embed/{embed.id}" title={talk.title} class="w-full h-full" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
								{:else if embed.type === 'vimeo'}
									<iframe src="https://player.vimeo.com/video/{embed.id}" title={talk.title} class="w-full h-full" frameborder="0" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen></iframe>
								{/if}
							</div>
						{:else}
						...
						{/if}
					{:else}
						<div class="aspect-video bg-gradient-to-br from-indigo-500 to-purple-700 flex items-center justify-center">
							<svg class="w-16 h-16 text-white/50" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M7 4v16M17 4v16M3 8h4m10 0h4M3 12h18M3 16h4m10 0h4M4 20h16a1 1 0 001-1V5a1 1 0 00-1-1H4a1 1 0 00-1 1v14a1 1 0 001 1z" /></svg>
						</div>
					{/if}
					<div class="p-4">
						<h3 class="font-semibold text-gray-900 dark:text-white">{talk.title}</h3>
						<div class="mt-1 text-sm text-gray-600 dark:text-gray-400">
							{#if talk.event}<span>{talk.event}</span>{/if}
							{#if talk.date}<span class="text-gray-400 dark:text-gray-500"> · </span>{formatDate(talk.date, { month: 'short', year: 'numeric' })}{/if}
						</div>
					...
					<div class="flex flex-col lg:flex-row">
						...
						{:else}
							<div class="lg:w-2/5 aspect-video bg-gradient-to-br from-indigo-500 to-purple-700 flex-shrink-0 flex items-center justify-center">
								<svg class="w-20 h-20 text-white/50" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M7 4v16M17 4v16M3 8h4m10 0h4M3 12h18M3 16h4m10 0h4M4 20h16a1 1 0 001-1V5a1 1 0 00-1-1H4a1 1 0 00-1 1v14a1 1 0 001 1z" /></svg>
							</div>
						{/if}

						<div class="flex-1 p-6">
							<h3 class="text-xl font-semibold text-gray-900 dark:text-white">{talk.title}</h3>
