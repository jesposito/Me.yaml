...
- ✅ Phase 0 (Foundation), Phase 1 (Content), Phase 1.5 (Discovery/Navigation), Phase 2 (View system with overrides), Phase 3 (Share tokens), and Phase 6 accents/themes are implemented.
- ✅ Media library (Phase 7.1) shipped: `/admin/media` with listing, filters, deletes.
- 🔜 Phase 7.2–7.4: image optimization (thumb/WebP/srcset), orphan cleanup UX (bulk delete/storage usage), external media embeds.
- 🔜 Phase 8: Security & Audit (audit log, headers, 2FA, session management).
- 🔜 Phase 9: Performance/SEO/Error UX polish.
- 🔜 Phase 10: Demo mode toggle (showcase persona with five views, backup/restore on toggle).
- 🧪 Testing backlog: frontend/integration tests, GitHub/AI mocks.

---

## Archive: Legacy Milestones (kept for reference)
