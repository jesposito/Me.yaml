...
func RegisterSeedHook(app *pocketbase.PocketBase) {
	...
	app.OnServe().BindFunc(func(se *core.ServeEvent) error {
		...
		default:
			log.Printf("Unknown SEED_DATA value: %s (supported: 'dev', 'minimal', 'demo')", seedMode)
		}
		if err != nil {
...
func seedDemoData(app *pocketbase.PocketBase) error {
	...
	// Create dedicated demo user (not the admin!)
	if err := createDemoUser(app); err != nil {
	...
	log.Println("  Posts: 2, Talks: 2, Awards: 1")
